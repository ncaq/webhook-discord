class MessageBuilder {
   ...
   */
  removeField(name) {
    this.data.attachments[0].fields = this.data.attachments[0].fields.filter(
  ...
  setImage(imageURL) {
    if (!imageURL) {
      process.emitWarning(
        "Image passed was null, nothing will be displayed in Discord"
      );
    }
  ...
  setThumbnail(thumbnailURL) {
    if (!thumbnailURL) {
      process.emitWarning(
        "thumbnail passed was null, nothing will be displayed in Discord"
      );
    }
