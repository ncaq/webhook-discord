...
class Webhook {
  ...
  verifyName(name) {
    if (name.length > 32) {
      throw new Error(
        `The name specified was ${name.length} characters. Discord webhook names must be under 32 characters`
      );
    }
  }
  ...
  checkFieldValueLength(value) {
    if (value.length > 1024)
      throw new Error(
        `The value given was ${value.length} characters, fields of Discord embeds must be under 1024 characters`
      );
  }
...
Webhook.prototype.custom = util.deprecate(
  Webhook.prototype.custom,
  "Custom is deprecated, use MessageBuilder instead"
);
